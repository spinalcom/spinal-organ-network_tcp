...
import { InputTCPDataEndpoint, InputTCPDataDevice } from './InputTCPData';
import {
 ...
 * @property {Array<InputDataEndpoint>} children
...
export class InputDataDevice implements idDevice {
  ...
  update(json: InputTCPDataDevice) {
    this.id = json.dp_bim_prefix;
    this.name = json.dp_bim_prefix;

    for (let i = 0; i < json.data.length; i += 1) {
      let endpoint = this.searchInData(json.data[i]);
      if (endpoint === null) {
        endpoint = new InputDataEndpoint();
        this.children.push(endpoint);
      }
      endpoint.update(json.data[i]);
    }
  }
  private searchInData(json: InputTCPDataEndpoint) {
    for (let i = 0; i < this.children.length; i += 1) {
      const endpoint = this.children[i];
      if (endpoint.id === json.dp_bim_postfix) {
        return endpoint;
      }
    }
    return null;
